...
teach_avg_acc_line = axs['teach_dist'].axvline(sum([b.get_height()*a.get_height() for b,a in zip(teach_dist_bars,teach_rel_bars)])*10-0.5,color="black",linestyle="-",linewidth=2)
teach_avg_conf_line = axs['teach_dist'].axvline(sum([b.get_height()*c for b,c in zip(teach_dist_bars,cal_bin_accs)])*10-0.5,color="gray",linestyle="--",linewidth=2)
...
def teach_rel_update(val):
    # get the new deviations from perfect calibration
    normal_weights = normal_density(teach_rel_mu.val,teach_rel_sigma.val,np.linspace(1/(2*n_bins),1-1/(2*n_bins),n_bins))
    ...
    teach_avg_acc_line.set(xdata=acc*10-0.5)
...
policies = np.linspace(0,1,11) # to increase granularity, we need more bins
conf_tradeoffs = []
for p in policies:
